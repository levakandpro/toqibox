...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // Отправляем сообщение в Telegram (без inline-клавиатуры для надежности)
    ...
    try {
      ...
      const telegramData = await messageResponse.json().catch(() => ({}));

      if (!messageResponse.ok) {
        console.error('[notify-payment-request] Telegram API error:', {
      ...
      console.log('[notify-payment-request] ✅ Message sent successfully to Telegram');
