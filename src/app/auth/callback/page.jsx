...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      // Проверяем только по email (таблица admins может быть недоступна из-за RLS или других проблем)
      // Если нужна проверка через таблицу admins, настройте RLS политики правильно
      ...
      // ВАЖНО: Если таблица admins недоступна (ошибка 500), проверка через таблицу пропущена.
      // Это не критично - проверка по email работает всегда.
      // Чтобы включить проверку через таблицу, исправьте RLS политики для таблицы admins.
